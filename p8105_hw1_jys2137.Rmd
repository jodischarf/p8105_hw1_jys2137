...
* The variable `vec_factor`. _As with the case above above, this is neither numeric nor logical. We get a warning message and a subsequent output of `NA`._
...
* Note: the code for `samp` works because it _is_ numeric.
...
* Converting `vec_char` to numeric **does not work**. We get an output of NAs because the values cannot be interpreted as numeric, which helps explain why we could not take the mean.
...
These `r ncol(penguins)` variables include descriptive characteristics (including the `species`, `sex`, `island`, and `year` of data collection) as well as the body measurements (including body mass and information about the bill and flippers).
